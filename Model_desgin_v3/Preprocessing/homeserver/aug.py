...
WORKERS_PER_FILE = 1    # × 2 files = 4 total → must equal OLLAMA_NUM_PARALLEL
CHECKPOINT_EVERY = 1
...
def count_existing(path: str) -> int:
    ...
    try:
        # Skip malformed rows, be more tolerant
        df = pd.read_csv(
            path,
            on_bad_lines='skip',          # ← key: ignore bad rows
            encoding='utf-8',
            low_memory=False,
            quoting=1                     # QUOTE_MINIMAL = 0, but try 1=QUOTE_ALL if needed
        )
        rows = len(df)
